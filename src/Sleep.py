...
class Sleep(CleanData):

    def __init__(self, *args, **kwargs):
        super(Sleep,self).__init__(*args, **kwargs)
        self.clean_csv = self.data_dir + 'Sleep_clean_data.csv' # name of merged dataframe
        self.clean_csv_w_states = self.data_dir + 'Sleep_clean_data_w_state.csv' # name of merged dataframe
        self.sdf_csv = self.data_dir + 'Sleep_Tract_Data.csv'
        self.tract_subset = {
            'GeographicLevel' :'Census Tract', #select only tract level data
            'MeasureId' : 'SLEEP', #select only binge drinking data
...
if __name__ =='__main__':
    data_cleaner = Sleep(vars_to_pull, is_test=True)
    ...
    img_dir = home + '../images_sleep/' # where images live
    data_dir = home + '../data/' # where data live
    merged_csv = data_dir + 'Merged_data.csv' # original data by tract (to get N)
    clean_csv = data_dir + 'Sleep_clean_data_w_state.csv' # cleaned data to analyze
    ...
    lrd.plot_state_coeffs(title='Impact of State on Sleep')